...
const sendEmailNotification = async (feedId: string, authorId: string, comment?: string) => {
  ...
  const postLink = `https://${feed.org.siteName}.Learnova.com/courses/${feed.courseId}?feedId=${feed.id}`;

  if (comment) {
    const emailData = [
      {
        from: `"${feed.org.name} - Learnova" <notify@mail.Learnova.com>`,
