...
export const load = async ({ url, cookies, request }): Promise<LoadOutput> => {
  ...
  if (isURLCustomDomain(url)) {
    ...
    if (!response.org) {
      throw redirect(307, 'https://peopletalk.io/404?type=org');
    }
    ...
    throw redirect(307, 'https://peopletalk.io');
  }
...
function isURLCustomDomain(url: URL) {
  ...
  const notCustomDomainHosts = [env.PRIVATE_APP_HOST || '', 'Learnova.com', 'vercel.app'].filter(
    Boolean
  );
