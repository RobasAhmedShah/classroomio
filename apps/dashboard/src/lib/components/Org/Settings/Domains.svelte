<script lang="ts">
  ...
  async function handleSaveCustomDomain() {
    ...
    if (sanitizedDomain.includes('Learnova')) {
      errors.customDomain = $t('components.settings.domains.custom_domain_not_Learnova');
      return;
    }
  ...
  <Row class="border-bottom-c py-7">
    ...
    <Column sm={2} md={6} lg={8}>
      ...
      <div>
        ...
        <TextField
          label="URL"
          helperMessage={`https://${siteName || ''}.Learnova.com`}
          bind:value={siteName}
          type="text"
